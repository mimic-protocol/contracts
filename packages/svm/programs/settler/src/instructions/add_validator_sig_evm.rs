...
#[derive(Accounts)]
pub struct AddValidatorSigEvm<'info> {
    #[account(mut)]
...
pub fn add_validator_sig_evm(ctx: Context<AddValidatorSigEvm>) -> Result<()> {
    ...
    require!(
        secp256k1_ix_args.msg == intent.hash,
    ...
    //     Pubkey::new_from_array(*secp256k1_ix_args.eth_address),
    ...
    // TODO: fix this in Intent layout as this is no longer a Pubkey

    let secp256k1_pubkey = Pubkey::try_from_slice(secp256k1_ix_args.eth_address)?;

    if intent.validators.contains(&secp256k1_pubkey) {
        return Ok(());
    }

    intent.validators.push(secp256k1_pubkey);

...
library IntentsHelpers {
    ...
    bytes32 internal constant VALIDATION_TYPE_HASH = keccak256('Validation(bytes32 intentHash)');
    ...
    function hash(Validation memory validation) internal pure returns (bytes32) {
        return keccak256(abi.encode(VALIDATION_TYPE_HASH, validation.intentHash));
    }
}

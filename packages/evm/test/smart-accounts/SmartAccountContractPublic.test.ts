...
import { toAddress } from '../helpers/index.js'

const { ethers } = await network.connect()

/* eslint-disable no-secrets/no-secrets */

describe('SmartAccountContractPublic', () => {
  ...
  beforeEach('deploy smart account', async () => {
    ...
    smartAccount = await ethers.deployContract('SmartAccountContractPublic', [settler.address, owner.address])
  })
    ...
    context('when the sender is authorized', () => {
      ...
      it('reverts', async () => {
        await expect(smartAccount.setAllowedSigners([], [])).to.be.revertedWithCustomError(
          smartAccount,
          'SmartAccountSetAllowedSignersDisabled'
        )
      })
    })
